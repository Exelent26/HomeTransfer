...
public class ISITWEEKEND {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        ...
        int daydo =(dateTime.getDayOfWeek()).getValue();
        int timedo = dateTime.getHour();
        if((daydo == FRIDAY && timedo >= WEEKEND_START_FRIDAY_CUT_OFF_HOUR) || (daydo == SATURDAY) || (daydo == SUNDAY && timedo <= WEEKEND_END_SUNDAY_CUT_OFF_HOUR)) {
            return true;
        }else {
            return false;
        }
